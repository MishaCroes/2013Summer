...
public class xacUpDownSenseFeatureMaker {
	...
	static final int NUMTOSEND = DuetTechExtension.WATCHACCELFPS;
...
//		Log.d(tag, "The " + (pntrEntry+1) + "th entry added");
	...
	public static int doClassification() {
		int idxClass = 0;
		Object[] features = getFlattenedData(NUMTOSEND);
		if (features != null) {
			try {
				idxClass = (int) UpDownSenseClassifier.classify(features);
			} catch (Exception e) {
