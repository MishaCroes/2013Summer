...
public class CrossManager {
	...
	// public static void addShape(xacInteractiveCanvas canvas) {
	//
	// if (canvasPhone == null || canvas != canvasPhone)
	// return;
	//
	// float dimPhone = 256;
	// canvasPhone.addShape(xacShape.OVAL, dimPhone, dimPhone);
	// // canvasWatch.addShape(xacShape.OVAL, dimPhone / PHONETOWATCHSIZERATIO,
	// // dimPhone / PHONETOWATCHSIZERATIO);
	//
	// // syncTheWatch();
	// }
	...
	public static void syncTheWatch() {
		...
		for (int i = 0; i < numShapesPhone; i++) {
			...
			float yWatch = shapePhone.getY()
					...
					- (canvasPhone.getHeight() / PHONETOWATCHSIZERATIO - canvasWatch
							.getHeight() / 2);
	...
	public static void syncToPhone() {
		...
		for (int i = 0; i < numShapesWatch; i++) {
			...
			float xPhone = (shapeWatch.getX() - canvasWatch.getWidth())
					* PHONETOWATCHSIZERATIO;
			float yPhone = (shapeWatch.getY() + (canvasPhone.getHeight()
