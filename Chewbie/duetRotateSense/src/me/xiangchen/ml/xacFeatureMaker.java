...
import me.xiangchen.app.rotatesense.RotateSense;
import me.xiangchen.app.rotatesense.RotateSenseExtension;
...
public class xacFeatureMaker {
	...
	public static void sendOffData(int numToSend, String[] classLabels,
		...
		int numToSendWatch = numToSendPhone
				* RotateSenseExtension.WATCHACCELFPS
				/ RotateSense.PHONEACCELFPS;
		...
		if (withWatch) {
			...
			for (int i = lockedPntrEntryWatch - numToSendWatch; i < lockedPntrEntryWatch; i++) {
				for (int j = 0; j < numFeatures; j++) {
					strFeatureRow += String.format("%.2f",
							featureTableWatch[i][j]) + ",";
				}
			}
		}
	...
	public static Object[] getFlattenedData(int numToSend, boolean withWatch) {
		int lockedPntrEntryPhone = pntrEntryPhone;
		int lockedPntrEntryWatch = pntrEntryWatch;
		int numToSendPhone = numToSend;
		int numToSendWatch = numToSendPhone
		...
		Object[] flattened = new Object[(numToSendPhone + (withWatch ? numToSendWatch : 0))
				* numFeatures];
		int idxFeature = 0;
