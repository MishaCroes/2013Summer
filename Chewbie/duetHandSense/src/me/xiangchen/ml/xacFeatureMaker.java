...
import me.xiangchen.app.duethandsense.HandSense;
import me.xiangchen.app.duethandsense.HandSenseExtension;
...
public class xacFeatureMaker {
	...
	public static boolean sendOffData(int numToSend, float[] extras,
			String[] classLabels) {
		int lockedPntrEntryPhone = pntrEntryPhone;
		int lockedPntrEntryWatch = pntrEntryWatch;
		int numToSendPhone = numToSend;
		int numToSendWatch = numToSendPhone * HandSenseExtension.WATCHACCELFPS
				/ HandSense.PHONEACCELFPS;
	...
	public static Object[] getFlattenedData(int numToSend, float[] extras) {
		...
		Object[] flattened = new Object[(numToSendPhone + numToSendWatch)
				* numFeatures + (extras == null ? 0 : extras.length)];
		int idxFeature = 0;
